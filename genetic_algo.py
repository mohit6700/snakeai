...
def cal_pop_fitness(pop):
    ...
    for i in range(pop.shape[0]):
        fit, score = run_game_with_ML(display,clock,pop[i])
        print('fitness value of chromosome '+ str(i) +' :  ' + str(fit) + ' score = ' + str(score) )
        fitness.append(fit)
        score_all.append(score)
    return np.array(fitness),np.array(score_all)
